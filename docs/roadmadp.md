...
1. M3a4 port は実装・検証・独立レビュー・コミット済み
   （clean baseline: `7982cc5`）
2. Graph / Flowchart の M3a 系はこれで完了。次は Sequence（participant kind、
   note / activation / create・destroy、fragment、arrow 種別）の詳細設計から
   開始する
3. 実装後は別担当の独立レビューと root 総合レビューを行い、既存 golden 差分0を確認する
4. Sequence -> State -> Class -> ER の順で M3 を完了する
