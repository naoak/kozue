...
1. Graph / Flowchart
   ...
   - **M3a2 次候補**:
     - **M3a2b 次工程（詳細設計済み、未着手）**: edge semantics / presentation
       - schema V6。旧 `Edge::new(..., ArrowType)` と legacy `arrow` wire bytes は維持
       - directed / undirected と source / target marker を型付け
       - Solid / Dashed / Dotted と Normal / Thick を意味から分離
       - native canonical modifier と Mermaid の限定 token subsetを追加
       - Scene path、layout retraction、全 backend、M3x0 contractを同時更新
       - 既存 golden 差分0、新規 edge presentation goldenのみ追加
     - M3a3 subgraph / container
     - M3a4 port
2. Sequence
   - participant kind
...
1. clean commit `df89ebc` から M3a2b edge semantics / presentation の実装を開始する
2. 詳細計画どおり IR/schema V6 -> Scene stroke -> frontend -> layout -> 全 backend の順で進める
3. 実装後は別担当の独立レビューと root 総合レビューを行い、既存 golden 差分0を確認する
4. 続いて M3a3 subgraph / container、M3a4 port へ進む
5. Graph 完了後は Sequence -> State -> Class -> ER の順で M3 を完了する
