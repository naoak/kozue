...
- M2 時点の serialize は V2（M3a1 で V3、M3a2a-I で V4、M3a2a-II で V5、M3a2b で V6 へ更新）、未知 version、必須 field 欠落、
  未知 nested field を拒否
...
状態: **M3a2b（Graph edge semantics / presentation）実装済み**
...
1. M3a2b edge semantics / presentation は実装・検証済み。コミット後の clean
   baseline（現在の `df89ebc` に代わる新しいコミットハッシュ）に更新すること
2. 次は M3a3 subgraph / container の詳細設計・実装から開始する
