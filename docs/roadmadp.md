...
- M2 時点の serialize は V2（M3a1 で V3 へ更新）、未知 version、必須 field 欠落、
  未知 nested field を拒否
...
1. Graph / Flowchart
   ...
   - **M3a2 次候補**:
     - node shape、subgraph / container、port
...
1. M3a1 の worktree diff を確認し、milestone 規約に沿って commit する
2. M3a2 の node shape と edge presentation の detailed design を作成する
3. shape-aware endpoint clipping と Scene primitive / stroke contractを確定する
4. annotation 構文対応前に `IrDocument` を layout / renderer へ渡す API を設計する
5. relation / message / transition 自体の安定 ID 生成規則を決める
